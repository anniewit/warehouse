...
public class HillClimbing {
    ...
    */
    public static boolean[] bestState(Order o, boolean[] currentState) throws Exception{
        ...
        for(int i = 0; i < stateLength; i++){
            firstNeighbour = neighbour(currentState,i);
            if(StateMethods.valid(o, firstNeighbour) == true){
                validStates.add(i);
            }
        }
        while(x < validStates.size() - 1){
            ...
            firstNeighbour = neighbour(currentState,position);
            int neighbourOne = StateMethods.countPsus(firstNeighbour);
            x++;
            position = validStates.get(x);
            secondNeighbour = neighbour(currentState,position);
    ...
    */
    public static boolean[] neighbour(boolean[] someState, int n) {
